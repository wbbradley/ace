...
void append(constraints_t &constraints,
  ...
  if (dbg_show_constraints) {
    log_location(info.location, "constraining a: %s b: %s because %s", a->str().c_str(),
                 b->str().c_str(), info.reason.c_str());
...
types::type_t::ref infer_core(expr_t *expr, env_t &env, constraints_t &constraints) {
  ...
  if (auto literal = dcast<literal_t *>(expr)) {
    ...
    auto tv = lambda->param_type != nullptr ? lambda->param_type
    ...
    if (lambda->return_type != nullptr) {
      append(constraints, return_type, lambda->return_type,
             {string_format("return type does not match type annotation :: %s",
    ...
    append(
        ...
        {string_format("(%s :: %s) applied to (%s :: %s) results in type %s",
                       ...
                       application->b->str().c_str(), t2->str().c_str(), tv->str().c_str()),
    ...
    append(constraints, type_arrow(fix->get_location(), tv, tv),
           infer(fix->f, env, constraints), {"fixpoint", fix->get_location()});
    return tv;
    ...
    append(constraints, t1, type_bool(condition->cond->get_location()),
           {"conditions must be bool", condition->get_location()});
    append(constraints, t2, t3,
           {"both branches of conditionals must match types with each other",
            condition->falsey->get_location()});
    return t2;
    ...
    append(constraints, t1, type_bool(while_->condition->get_location()),
           {"while conditions must be bool", while_->condition->get_location()});
    ...
    append(constraints, t1, env.return_type,
           {string_format("returning (%s " c_good("::") " %s and %s)",
    ...
    append(constraints, t1, tuple,
           {string_format("dereferencing tuple index %d of %d", tuple_deref->index,
                          tuple_deref->max),
            expr->get_location()});
    ...
    append(
        constraints, t1, type_arrows(ts),
        {string_format("builtin %s", builtin->var->str().c_str()), builtin->get_location()});
    return ts.back();
  } else if (auto as = dcast<as_t *>(expr)) {
    auto t1 = infer(as->expr, env, constraints);
    auto as_type = as->scheme->instantiate(as->get_location());
    if (!as->force_cast) {
      append(constraints, t1, as_type,
             {string_format("we can get type %s from %s", as->scheme->str().c_str(),
                            as->expr->str().c_str()),
              as->get_location()});
    }
    return as_type;
    ...
    for (auto pattern_block : match->pattern_blocks) {
      ...
      append(
          constraints, tp, t1,
          {"pattern must match type of scrutinee", pattern_block->predicate->get_location()});

      auto t2 = infer(pattern_block->result, local_env, constraints);
      if (match_type != nullptr) {
        append(constraints, t2, match_type,
  ...
  throw user_error(expr->get_location(), "unhandled inference for %s", expr->str().c_str());
}
...
types::type_t::ref irrefutable_predicate_t::infer(env_t &env,
...
types::type_t::ref ctor_predicate_t::infer(env_t &env, constraints_t &constraints) const {
  ...
  if (ctor_params.size() - 1 != params.size()) {
    throw user_error(
        get_location(), "incorrect number of sub-patterns given to %s (%d vs. %d) %s %s",
        ctor_name.str().c_str(), ctor_params.size() - 1, params.size(),
        ctor_params.back()->str().c_str(), ::join_str(ctor_params, ", ").c_str());
  }
...
std::string constraint_t::str() const {
  return string_format("%s == %s because %s", a->str().c_str(), b->str().c_str(),
                       info.str().c_str());
}
