...
void append(constraints_t &constraints,
  ...
  if (dbg_show_constraints) {
    log_location(info.location, "constraining a: %s b: %s because %s",
                 a->str().c_str(), b->str().c_str(), info.reason.c_str());
...
types::type_t::ref infer_core(expr_t *expr,
  ...
  if (auto literal = dcast<literal_t *>(expr)) {
    ...
    append(
        constraints, t1, type_bool(while_->condition->get_location()),
    ...
    append(constraints, t1, tuple,
           {string_format("dereferencing tuple index %d of %d",
    ...
    if (!as->force_cast) {
      append(constraints, t1, as_type,
             {string_format("we can get type %s from %s",
