...
#define expect_token(_tk)                                                                    \
  do {                                                                                       \
    if (ps.token.tk != _tk) {                                                                \
      ps.error("expected '%s', got '%s' " c_id("%s"), tkstr(_tk), tkstr(ps.token.tk),        \
  ...
  do {                                                                                       \
    ...
    if (ps.token.tk != tk_identifier || ps.token.text != token_text) {                       \
      ps.error("expected " c_id("%s") ", got " c_warn("%s"), token_text,                     \
               ps.token.text.size() != 0 ? ps.token.text.c_str() : tkstr(ps.token.tk));      \
    }                                                                                        \
  } while (0)
...
bitter::module_t *parse_module(parse_state_t &ps,
