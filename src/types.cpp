...
std::shared_ptr<scheme_t> type_t::generalize(
  ...
  predicate_map_t predicate_map_t;
  type_t::map bindings;
  for (auto ftv : type_ftvs) {
    if (!in(ftv.first, pm)) {
      vs.push_back(ftv.first);
      mutating_merge(ftv, predicate_map_t);
      bindings[ftv.first] = type_variable(make_iid(ftv.first));
    }
  }
  return scheme(vs, predicate_map_t, rebind(bindings));
}
...
std::ostream &type_id_t::emit(std::ostream &os,
                              const map &bindings,
...
predicate_map_t type_id_t::get_predicate_map() const {
  return {};
}
...
type_t::ref type_id_t::remap_vars(
    const std::map<std::string, std::string> &map) const {
  return shared_from_this();
}

type_t::ref type_id_t::prefix_ids(const std::set<std::string> &bindings,
...
type_variable_t::type_variable_t(identifier_t id,
...
std::ostream &type_variable_t::emit(std::ostream &os,
                                    const map &bindings,
                                    int parent_precedence) const {
...
predicate_map_t type_variable_t::get_predicate_map() const {
  predicate_map_t pm;
...
type_t::ref type_variable_t::remap_vars(
    const std::map<std::string, std::string> &map) const {
...
type_t::ref type_variable_t::prefix_ids(const std::set<std::string> &bindings,
                                        const std::string &pre) const {
...
std::ostream &type_operator_t::emit(std::ostream &os,
                                    const map &bindings,
                                    int parent_precedence) const {
...
predicate_map_t type_operator_t::get_predicate_map() const {
...
type_t::ref type_operator_t::remap_vars(
    const std::map<std::string, std::string> &map) const {
  return ::type_operator(oper->remap_vars(map), operand->remap_vars(map));
}

type_t::ref type_operator_t::prefix_ids(const std::set<std::string> &bindings,
                                        const std::string &pre) const {
...
std::ostream &type_tuple_t::emit(std::ostream &os,
                                 const map &bindings,
                                 int parent_precedence) const {
  os << "(";
...
predicate_map_t type_tuple_t::get_predicate_map() const {
  predicate_map_t pm;
  for (auto dimension : dimensions) {
...
type_t::ref type_tuple_t::remap_vars(
    const std::map<std::string, std::string> &map) const {
  bool anything_was_rebound = false;
  refs type_dimensions;
  for (auto dimension : dimensions) {
...
type_t::ref type_tuple_t::prefix_ids(const std::set<std::string> &bindings,
                                     const std::string &pre) const {
  bool anything_was_rebound = false;
  refs type_dimensions;
  for (auto dimension : dimensions) {
...
std::ostream &type_lambda_t::emit(std::ostream &os,
                                  const map &bindings_,
                                  int parent_precedence) const {
  parens_t parens(os, parent_precedence, get_precedence());
...
predicate_map_t type_lambda_t::get_predicate_map() const {
  assert(false);
  return {};
#if 0
		map bindings;
...
type_t::ref type_lambda_t::prefix_ids(const std::set<std::string> &bindings,
                                      const std::string &pre) const {
...
type_t::map remove_bindings(const type_t::map &env,
...
predicate_map_t scheme_t::get_predicate_map() {
  predicate_map_t predicate_map_t = type->get_predicate_map();
  for (auto var : vars) {
    predicate_map_t.erase(var);
  }
  return predicate_map_t;
}
...
types::type_t::ref type_vector_type(types::type_t::ref element) {
  return type_operator(
      type_id(identifier_t{VECTOR_TYPE, element->get_location()}), element);
}
...
types::type_t::ref type_map(types::type_t::ref a, types::type_t::ref b) {
  return type_operator(
      type_operator(type_id(identifier_t{"Map", a->get_location()}), a), b);
}
...
types::type_t::ref type_arrow(location_t location,
                              ...
                              types::type_t::ref b) {
  return type_operator(
      type_operator(type_id(identifier_t{ARROW_TYPE_OPERATOR, location}), a),
      b);
}
...
types::type_t::ref type_ptr(types::type_t::ref raw) {
  return type_operator(
      type_id(identifier_t{PTR_TYPE_OPERATOR, raw->get_location()}), raw);
}
