...
Pattern::ref intersect(location_t location,
    ...
    for (auto &cpv : lhs) {
        ...
        intersection = pattern_union(
            std::accumulate(rhs.begin(), rhs.end(), init, cpv_intersect), intersection);
    }
    return intersection;
}
    ...
    if (auto tuple_type = dyncast<const types::type_tuple_t>(type)) {
        ...
        for (auto pair : ctors_terms) {
            ...
            for (size_t i = 0; i < ctor_terms.size() - 1; ++i) {
                args.push_back(std::make_shared<AllOf>(location, maybe<identifier_t>(), tenv,
                                                       ctor_terms[i]));
            }
            /* add a ctor */
    ...
    if (lhs_allof) {
        if (rhs_allof) {
            ...
            auto error = user_error(
                lhs->location, "type mismatch when comparing ctors for pattern intersection");
    ...
    if (lhs_ctor_patterns) {
        if (rhs_ctor_patterns) {
            for (auto &cpv : lhs_ctor_patterns->cpvs) {
                difference(std::make_shared<CtorPattern>(lhs_ctor_patterns->location, cpv),
                           rhs, send);
            }
            return;
