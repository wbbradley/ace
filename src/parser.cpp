...
ptr<var_decl_t> var_decl_t::parse_param(parse_state_t &ps) {
	...
	if (ps.token.tk == tk_assign) {
		throw user_error_t(ps.token.location, "default values for function arguments are not a thing");
...
ptr<statement_t> get_statement_parse(parse_state_t &ps) {
	...
	if (link_statement->link_as_name.tk != tk_identifier) {
		throw user_error_t(ps.token.location, "expected an identifier for link module name (either implicit or explicit)");
	}
	...
	if (ps.token.tk == tk_lsquare || ps.token.is_ident(K(def))) {
		...
		if (ps.token.is_ident(K(to))) {
			...
			if (ps.token.tk != tk_identifier && ps.token.tk != tk_string) {
				throw user_error_t(ps.token.location, "expected an identifier (or string) for the external name of the linked function (not %s)",
						ps.token.str().c_str());
			}
			expect_token(tk_identifier);
		...
		throw user_error_t(ps.token.location, "invalid link syntax (TODO: make this error better)");
	}
}
...
ptr<expression_t> reference_expr_t::parse(parse_state_t &ps) {
	if (ps.token.tk == tk_identifier) {
		...
		throw user_error_t(ps.token.location, "expected an identifier");
	}
}
...
ptr<expression_t> array_literal_expr_t::parse(parse_state_t &ps) {
	...
	while (ps.token.tk != tk_rsquare && ps.token.tk != tk_none) {
		...
		if (ps.token.tk == tk_comma) {
			...
			throw user_error_t(ps.token.location, "found something that does not make sense in an array literal");
		}
	}
...
ptr<expression_t> literal_expr_t::parse(parse_state_t &ps) {
	...
	case tk_indent:
		throw user_error_t(ps.token.location, "unexpected indent");

	case tk_identifier:
		if (ps.token.is_ident(K(def))) {
			return function_defn_t::parse(ps);
		}
		throw user_error_t(ps.token.location, "unexpected token found when parsing literal expression. '" c_error("%s") "'", ps.token.text.c_str());

	default:
		throw user_error_t(ps.token.location, "out of place token found when parsing literal expression. '" c_error("%s") "' (%s)",
			   	ps.token.text.c_str(),
				tkstr(ps.token.tk));
	}
}
...
std::vector<ptr<expression_t>> parse_param_list(parse_state_t &ps) {
	...
	while (ps.token.tk != tk_rparen) {
		...
		if (ps.token.tk == tk_comma) {
			...
			throw user_error_t(ps.token.location, "unexpected %s in parameter list (" c_id("%s") ")", tkstr(ps.token.tk), ps.token.text.c_str());
		}
	...
	if (ps.token.tk != tk_comma) {
		...
		while (ps.token.tk != tk_rparen) {
			...
			if (ps.token.tk == tk_comma) {
				...
				throw user_error_t(ps.token.location, 
...
#define handle_assign(tk_, type) \
	...
	if (!ps.line_broke() && ps.token.tk == tk_becomes) {
		if (lhs->sk == sk_reference_expr) {
			...
			throw user_error_t(ps.token.location, ":= may only come after a new symbol name");
		}
	} else {
		return lhs;
	}
...
ptr<param_list_decl_t> param_list_decl_t::parse(parse_state_t &ps) {
	...
	while (ps.token.tk != tk_rparen) {
		...
		if (ps.token.tk == tk_comma) {
			...
			throw user_error_t(ps.token.location, "unexpected token in param_list_decl");
		}
	}
...
ptr<block_t> block_t::parse(parse_state_t &ps) {
	...
	if (ps.token.tk == tk_outdent) {
		throw user_error_t(block->token.location, "empty blocks are not allowed, sorry. use pass.");
	}
	...
	while (ps.token.tk != tk_outdent) {
		...
		if (!ps.line_broke()
			...
			throw user_error_t(ps.token.location, "statements must be separated by a newline (or a semicolon)");
		}
...
ptr<if_block_t> if_block_t::parse(parse_state_t &ps) {
	...
	if (ps.token.is_ident(K(if)) || ps.token.is_ident(K(elif))) {
		...
		throw user_error_t(ps.token.location, "expected if or elif");
	}
	...
	if (auto condition = dyncast<const expression_t>(expression)) {
		...
		throw user_error_t(condition_token.location,
...
ptr<when_block_t> when_block_t::parse(parse_state_t &ps) {
	...
	if (when_block->pattern_blocks.size() == 0) {
		throw user_error_t(ps.token.location, "when block did not have subsequent patterns to match");
	}
...
ptr<function_decl_t> function_decl_t::parse(parse_state_t &ps) {
	...
	if (ps.token.tk == tk_lsquare) {
		...
		if (ps.token.is_ident(K(global))) {
			...
			throw user_error_t(ps.token.location, "expected module injection");
		}
	}
	...
	if (function_type->name == nullptr) {
		throw user_error_t(function_type->get_location(), "function is missing a name");
	} else {
	...
	if (name == "main") {
		if (extends_module == nullptr) {
			...
			throw user_error_t(attributes_location,
	...
	if (name == "__finalize__") {
		if (auto args = dyncast<const types::type_args_t>(function_type->args)) {
			if (args->args.size() != 1) {
				throw user_error_t(function_type->name->get_location(),
	...
	while (!ps.line_broke() && ps.token.tk == tk_identifier) {
		if (token_is_illegal_in_type(ps.token)) {
			if (ps.token.is_ident(K(any))) {
				throw user_error_t(ps.token.location, "`any` is unnecessary within type parameters of type declarations");
			}
		...
		std::vector<ptr<const ast::link_name_t>> linked_names)
{
	...
	for (auto link_name : linked_names) {
		...
		if (in(local_name, ps.type_macros)) {
			throw user_error_t(link_name->local_name.location,
