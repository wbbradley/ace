...
expr_t *parse_assign_tuple(parse_state_t &ps, tuple_t *tuple) {
    ...
    for (int i = refs.size() - 1; i >= 0; --i) {
        body = new let_t(
            refs[i].second,
            new application_t(new var_t(make_iid(string_format("__[%d]__", i))), rhs_var),
            body);
    }
...
expr_t *parse_new_expr(parse_state_t &ps) {
    ps.advance();
    return new as_t(
        new application_t(new var_t(ps.id_mapped({"new", ps.prior_token.location})),
            ...
            if (ps.token.tk == tk_rsquare) {
                ...
                if (ps.token.tk == tk_assign) {
                    ...
                    expr = new application_t(
                        new application_t(
                            new application_t(new var_t({"std.set_indexed_item", location}),
                                              expr),
                            start),
                        rhs);
                } else {
...
expr_t *parse_times_expr(parse_state_t &ps) {
    ...
    while (!ps.line_broke() &&
        ...
        expr =
            new application_t(new application_t(new var_t(op), expr), parse_prefix_expr(ps));
    }
...
expr_t *parse_bitwise_xor(parse_state_t &ps) {
    ...
    while (!ps.line_broke() && ps.token.tk == tk_hat) {
        ...
        expr =
            new application_t(new application_t(new var_t(op), expr), parse_bitwise_and(ps));
    }
    return expr;
}
...
expr_t *parse_bitwise_or(parse_state_t &ps) {
    ...
    while (!ps.line_broke() && ps.token.tk == tk_pipe) {
        ...
        expr =
            new application_t(new application_t(new var_t(op), expr), parse_bitwise_xor(ps));
    }
...
expr_t *parse_assignment(parse_state_t &ps) {
    ...
    if (ps.token.tk == tk_assign) {
        ...
        return new application_t(
            new application_t(new var_t(identifier_t{"std.store_value", ps.token.location}),
                              lhs),
            rhs);
    }
    ...
    if (expression_block_syntax) {
        if (!ps.line_broke()) {
            ...
            if (ps.token.tk != tk_rparen && ps.token.tk != tk_rcurly &&
                ps.token.tk != tk_rsquare && ps.token.tk != tk_comma && !ps.line_broke()) {
                throw user_error(ps.token.location,
...
types::type_t::ref parse_type(parse_state_t &ps) {
    ...
    while (token_begins_type(ps.token) && !ps.line_broke()) {
        if (ps.token.tk == tk_lparen) {
            ...
            auto error =
                user_error(ps.token.location, "unhandled syntax for type specification");
