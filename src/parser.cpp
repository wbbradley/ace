...
ptr<var_decl_t> var_decl_t::parse(parse_state_t &ps, bool is_let) {
	...
	if (ps.token.tk != tk_assign) {
		var_decl->type = parse_type(ps, {} /*generics*/);
	} else {
...
ptr<typeinfo_expr_t> typeinfo_expr_t::parse(parse_state_t &ps) {
	...
	ps.advance();
	if (!!ps.status) {
		typeinfo_expr->type = parse_type(ps, {});
		if (!!ps.status) {
			chomp_token(tk_comma);

			typeinfo_expr->underlying_type = parse_type(ps, {});
...
ast::pattern_block_t::ref pattern_block_t::parse(parse_state_t &ps) {
	...
	auto pattern_type = parse_type(ps, {});
		...
		identifier::refs type_variables_list)
{
	...
	auto type = parse_type(ps, type_variables);
		...
		bool native)
{
	...
	auto type = parse_type(ps, generics);
