...
bool scheme_equality(types::scheme_t::ref a, types::scheme_t::ref b) {
  ...
  if (!unification.result) {
    debug_above(3,
                log_location(unification.error_location,
                             "schemes %s and %s do not match because %s", ta->str().c_str(),
                             tb->str().c_str(), unification.error_string.c_str()));
  }
...
bool type_equality(types::type_t::ref a, types::type_t::ref b) {
  ...
  if (auto ti_a = dyncast<const type_id_t>(a)) {
    ...
    auto error = user_error(a->get_location(),
...
unification_t bind(std::string a, type_t::ref type, const std::set<std::string> &instances) {
  if (occurs_check(a, type)) {
    return unification_t{
        false,
        type->get_location(),
        string_format("infinite type detected! %s = %s", a.c_str(), type->str().c_str()),
        {},
        {}};
  }
  ...
  debug_above(6, log("binding type variable %s to %s gives bindings %s", a.c_str(),
...
unification_t unify(type_t::ref a, type_t::ref b) {
  ...
  if (auto tv_a = dyncast<const type_variable_t>(a)) {
    ...
    if (auto to_b = dyncast<const type_operator_t>(b)) {
      return unify_many({to_a->oper, to_a->operand}, {to_b->oper, to_b->operand});
    }
  } else if (auto tup_a = dyncast<const type_tuple_t>(a)) {
    if (auto tup_b = dyncast<const type_tuple_t>(b)) {
...
types::type_t::map solver(const types::type_t::map &bindings,
  ...
  if (unification.result) {
    ...
    auto error =
        user_error(unification.error_location, "%s", unification.error_string.c_str());
...
types::type_t::map compose(const types::type_t::map &a, const types::type_t::map &b) {
  ...
  for (auto pair : a) {
    debug_above(11, log("-- check %s in %s when going to assign it to %s -- ",
                        pair.first.c_str(), str(m).c_str(), pair.second->str().c_str()));
    assert(!in(pair.first, m));
    m[pair.first] = pair.second;
  }
  debug_above(11, log("which gives: %s", join_with(m, ", ", [](const auto &pair) {
                                           return string_format("%s: %s", pair.first.c_str(),
                                                                pair.second->str().c_str());
                                         }).c_str()));
  return m;
...
unification_t compose(const unification_t &a, const unification_t &b) {
  if (a.result && b.result) {
    auto unification =
        unification_t{true, INTERNAL_LOC(), "", compose(a.bindings, b.bindings), {}};
