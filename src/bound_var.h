...
struct bound_var_t : public var_t {
	typedef std::shared_ptr<const bound_var_t> ref;
	...
	virtual std::shared_ptr<bound_var_t> this_bound_var() = 0;
	virtual std::shared_ptr<const bound_var_t> this_bound_var() const = 0;
	virtual llvm::Value *get_llvm_value(std::shared_ptr<scope_t> scope) const = 0;
	...
	virtual llvm::Value *llvm_dereferencing_load(std::shared_ptr<scope_t> scope, llvm::IRBuilder<> &builder) const = 0;
	virtual ref dereferencing_load(llvm::IRBuilder<> &builder, std::shared_ptr<scope_t> scope) const = 0;
...
struct bound_module_t : public bound_var_t {
	virtual std::shared_ptr<module_scope_t> get_module_scope() const = 0;
};
