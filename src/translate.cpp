...
expr_t *texpr(const defn_id_t &for_defn_id,
  ...
  try {
    ...
    debug_above(6, log("monomorphizing %s to have type %s", expr->str().c_str(),
                       type->str().c_str()));
    if (auto literal = dcast<literal_t *>(expr)) {
      if (literal->token.tk == tk_string) {
        ...
        types::type_t::refs string_ref_terms = {
            type_operator(type_id(make_iid(PTR_TYPE_OPERATOR)), type_id(make_iid(CHAR_TYPE))),
            type_int(INTERNAL_LOC()),
            type_operator(type_id(make_iid(VECTOR_TYPE)), type_id(make_iid(CHAR_TYPE))),
        };
        ...
        auto new_str =
            new literal_t(token_t{literal->token.location, tk_string, literal->token.text});
        typing[new_str] =
            type_operator(type_id(make_iid(PTR_TYPE_OPERATOR)), type_id(make_iid(CHAR_TYPE)));
        ...
        expr_t *new_str_len = new literal_t(
            token_t{INTERNAL_LOC(), tk_integer,
                    string_format("%d", unescape_json_quotes(literal->token.text).size())});
      ...
      auto inner_expr = texpr(for_defn_id, static_print->expr, bound_vars, tenv, typing,
                              needed_defns, fake_returns);
      log_location(static_print->expr->get_location(), "type is %s",
                   typing[inner_expr]->str().c_str());
      log_location(static_print->get_location(), "while translating %s",
      ...
      if (!in(var->id.name, bound_vars)) {
        ...
        debug_above(6, log(c_id("%s") " depends on " c_id("%s"), for_defn_id.str().c_str(),
                           defn_id.str().c_str()));
      ...
      auto new_body = texpr(for_defn_id, lambda->body, new_bound_vars, tenv, typing,
      ...
      auto a =
          texpr(for_defn_id, application->a, bound_vars, tenv, typing, needed_defns, returns);
      auto b =
          texpr(for_defn_id, application->b, bound_vars, tenv, typing, needed_defns, returns);
      auto new_app = new application_t(a, b);
      typing[new_app] = type;
      return new_app;
    } else if (auto let = dcast<let_t *>(expr)) {
      auto new_value =
          texpr(for_defn_id, let->value, bound_vars, tenv, typing, needed_defns, returns);
      auto new_bound_vars = bound_vars;
      new_bound_vars.insert(let->var.name);
      auto new_body =
          texpr(for_defn_id, let->body, new_bound_vars, tenv, typing, needed_defns, returns);
      ...
      auto cond = texpr(for_defn_id, condition->cond, bound_vars, tenv, typing, needed_defns,
                        returns);
      bool truthy_returns = false;
      auto truthy = texpr(for_defn_id, condition->truthy, bound_vars, tenv, typing,
                          needed_defns, truthy_returns);
      bool falsey_returns = false;
      auto falsey = texpr(for_defn_id, condition->falsey, bound_vars, tenv, typing,
      ...
      auto condition = texpr(for_defn_id, while_->condition, bound_vars, tenv, typing,
                             needed_defns, returns);
      auto block = texpr(for_defn_id, while_->block, bound_vars, tenv, typing, needed_defns,
                         block_returns);
      /* NB: we don't really care if the block returns because we can't validate
       * that the loop ever actually runs */
      ...
      auto ret = new return_statement_t(texpr(for_defn_id, return_->value, bound_vars, tenv,
      ...
      for (auto dim : tuple->dims) {
        ...
        dims.push_back(
            texpr(for_defn_id, dim, bound_vars, tenv, typing, needed_defns, returns));
      }
      auto new_tuple = new tuple_t(tuple->get_location(), dims);
      typing[new_tuple] = type;
      return new_tuple;
    } else if (auto match = dcast<match_t *>(expr)) {
      return translate_match_expr(for_defn_id, match, bound_vars, tenv, typing, needed_defns,
                                  returns);
    } else if (auto as = dcast<as_t *>(expr)) {
      auto expr =
          texpr(for_defn_id, as->expr, bound_vars, tenv, typing, needed_defns, returns);
      if (as->force_cast) {
      ...
      auto new_builtin =
          new builtin_t(dynamic_cast<var_t *>(texpr(for_defn_id, builtin->var, bound_vars,
      ...
      auto new_tuple_deref =
          new tuple_deref_t(texpr(for_defn_id, tuple_deref->expr, bound_vars, tenv, typing,
                                  needed_defns, returns),
...
types::type_t::ref translation_env_t::get_type(const bitter::expr_t *e) const {
  ...
  if (t == nullptr) {
    log(log_error, "translation env does not contain a type for %s", e->str().c_str());
    assert(false && !!"missing type for expression");
  }
  return t;
}

types::type_t::refs translation_env_t::get_data_ctor_terms(types::type_t::ref type,
  ...
  debug_above(7, log("looking for %s in data_ctors_map of size %d", id->str().c_str(),
                     int(data_ctors_map.size())));
...
std::map<std::string, types::type_t::refs> translation_env_t::get_data_ctors_terms(
...
int translation_env_t::get_ctor_id(std::string ctor_name) const {
  ...
  if (iter == ctor_id_map.end()) {
    throw user_error(INTERNAL_LOC(), "bad ctor name requested during translation (%s)",
                     ctor_name.c_str());
  } else {
    return iter->second;
  }
