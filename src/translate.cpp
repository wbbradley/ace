    ...
    try {
        ...
        if (auto literal = dcast<literal_t *>(expr)) {
            if (literal->token.tk == tk_string) {
                ...
                auto defn_id =
                    defn_id_t{vector_ctor, vector_ctor_type->generalize({})->normalize()};
                insert_needed_defn(needed_defns, defn_id, literal->get_location(),
                                   for_defn_id);

                auto new_str = new literal_t(
                    token_t{literal->token.location, tk_string, literal->token.text});
                typing[new_str] = type_operator(type_id(make_iid(PTR_TYPE_OPERATOR)),
                                                type_id(make_iid(CHAR_TYPE)));

                expr_t *new_ctor_partial_1 = new application_t(ctor_var, new_str);
                typing[new_ctor_partial_1] = type_arrows(string_ref_terms, 1);

                expr_t *new_str_len = new literal_t(token_t{
                    INTERNAL_LOC(), tk_integer,
            ...
            auto new_value = texpr(for_defn_id, let->value, bound_vars, tenv, typing,
                                   needed_defns, returns);
            auto new_bound_vars = bound_vars;
            new_bound_vars.insert(let->var.name);
            auto new_body = texpr(for_defn_id, let->body, new_bound_vars, tenv, typing,
                                  needed_defns, returns);
            ...
            auto cond = texpr(for_defn_id, condition->cond, bound_vars, tenv, typing,
                              needed_defns, returns);
            ...
            auto condition = texpr(for_defn_id, while_->condition, bound_vars, tenv, typing,
                                   needed_defns, returns);
            auto block = texpr(for_defn_id, while_->block, bound_vars, tenv, typing,
            ...
            auto ret = new return_statement_t(texpr(for_defn_id, return_->value, bound_vars,
            ...
            return translate_match_expr(for_defn_id, match, bound_vars, tenv, typing,
                                        needed_defns, returns);
            ...
            auto new_tuple_deref =
                new tuple_deref_t(texpr(for_defn_id, tuple_deref->expr, bound_vars, tenv,
                                        typing, needed_defns, returns),
