...
typedef void (*addref_fn_t)(void *object);
typedef void (*release_fn_t)(void *object);
...
void release_fn_default(void *object, version_t version) {
	/* do nothing */
}
		...
		size_t object_size)
{
	...
	/* store the GC memory releaseing function */
	var->release_fn = release_fn;
...
void push_stack_var(struct var_t *var) {
	...
	// object.
	struct var_t *stack_ref_var = create_var("stack ref", NULL /*release_fn*/,
...
void release_stack_var(uint_fast64_t version, struct stack_ref_t *stack_ref) {
	...
	if (var->release_fn != NULL) {
		/* if this object has children, let's release them */
		(*var->release_fn)(VAR_DATA_ADDR(var), version);
	}

	print_var(c_var("releaseed"), var);
}
...
void *gc(void *context) {
	...
	/* begin by getting a base version, after which everything created or releaseed
	...
	while (thread != NULL) {
		 ...
		 * that as a starting place for the subsequent release and sweep */
		struct var_t *var = create_var(c_internal("sentinel"), NULL /*release_fn*/,
		...
		struct stack_ref_t *stack_ref = atomic_load(&thread->head_stack_ref);
		while (stack_ref != NULL) {
			release_stack_var(gc_version, stack_ref);
			stack_ref = stack_ref->next_stack_ref;
		}
	...
	while (thread != NULL) {
		...
		while (next_var_handle->var != NULL) {
			...
			if (version == 0) {
				...
				/* this object is old and stale despite our release pass above */
	...
	while (1) {
		...
		if (linelen >= 0) {
			...
			if (strcmp(line, "pop") == 0) {
				...
				var = create_var(line, NULL /*release_fn*/, 1 /* type_id (fake) */,
					   	sizeof(zion_int_t));
