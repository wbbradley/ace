...
ptr<var_decl> var_decl::parse(parse_state_t &ps) {
	...
	if (ps.token.tk != tk_assign) {
		...
		var_decl->type_ref = create<ast::type_ref_generic>(ps.token, types::term_generic());
	}
...
#define handle_assign(tk_, type) \
		...
		if (!ps.line_broke() && ps.token.tk == tk_becomes) {
			if (lhs->sk == sk_reference_expr) {
				...
				var_decl->type_ref = make_ptr<type_ref_generic>(
						types::term_generic());
	   	...
	   	int depth)
{
	...
	while (!!ps.status && (ps.token.tk == tk_identifier || ps.token.tk == tk_any)) {
		...
		auto next_term = parse_term(ps, generics, depth);
		if (!!ps.status) {
	...
	case tk_identifier:
		{
			...
			types::term::refs arguments;
			if (ps.token.tk == tk_lcurly) {
				arguments = parse_term_arguments(ps, generics, depth + 1);
			}
