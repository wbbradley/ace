...
ptr<var_decl> var_decl::parse(parse_state_t &ps) {
	...
	if (ps.token.tk != tk_assign) {
		...
		var_decl->type_ref = create<ast::type_ref_standard>(ps.token, type_variable());
	}
...
#define handle_assign(tk_, type) \
		...
		if (!ps.line_broke() && ps.token.tk == tk_becomes) {
			if (lhs->sk == sk_reference_expr) {
				...
				var_decl->type_ref = make_ptr<type_ref_standard>(
						type_variable());
	...
	do {
		if (ps.token.tk == tk_lsquare) {
			...
			if (!!ps.status) {
				...
				|| ps.token.tk == tk_lcurly)
	   	{
			auto type_ref = type_ref_standard::parse(ps, generics);
			if (!!ps.status) {
				type_refs.push_back(type_ref);
			}
		} else {
...
type_ref::ref type_ref_standard::parse(parse_state_t &ps, identifier::set generics) {
	assert(ps.token.tk != tk_any);
	return create<ast::type_ref_standard>(ps.token, parse_maybe_type(ps, generics));
}
